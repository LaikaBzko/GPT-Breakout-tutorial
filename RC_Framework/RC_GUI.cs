...
namespace Tutorial_1.RC_Framework
{
    ...
    public class GUI_Globals
    {
        ...
        public static void drawToolTip(SpriteBatch sb)
        {
            if (toolTipText != "")
            {
                Vector2 size = tTfont.MeasureString(toolTipText);

                sb.Draw(whiteTex, new Rectangle((int)tipPos.X, (int)tipPos.Y, (int)size.X, (int)size.Y), tTColorBack);
                sb.DrawString(tTfont, toolTipText, tipPos, tTColor);
            }
        }
